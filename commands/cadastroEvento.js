...
async function cadastrarEvento(msg, client) {
  ...
  if (horarioEscolha === "1") horario = horarios.MANHÃ;
  else if (horarioEscolha === "2") horario = horarios.TARDE[0];
  else horario = horarios.NOITE.OUTROS;
  ...
  if (!dataFormatada) {
    await client.sendMessage(chatId, "⚠️ Data inválida. Use o formato DD/MM/AAAA.");
    return;
  }
  ...
  if (duplicado) {
    await client.sendMessage(chatId, "⚠️ Esse evento já foi cadastrado.");
    return;
  }
  ...
  if (confirmar === "1") {
    ...
    await client.sendMessage(chatId, "❌ Cadastro cancelado.");
  }
}
...
function formatarData(str) {
  ...
  return isNaN(data) ? null : data.toISOString().split("T")[0];
}
